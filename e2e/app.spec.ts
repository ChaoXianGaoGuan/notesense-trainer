...
test('each module renders and supports a basic answer flow', async ({ page }) => {
  ...
  await expect(syncopationModule.getByRole('button', { name: '四分音符' })).toHaveClass(/active/)
  await expect(syncopationModule.getByRole('button', { name: '二八节奏' })).toBeVisible()
  await expect(syncopationModule.getByRole('button', { name: '四十六节奏' })).toBeVisible()
  await expect(syncopationModule.getByRole('button', { name: '前八后十六/前十六后八' })).toBeVisible()
  await expect(syncopationModule.getByRole('button', { name: '前附点/后附点' })).toBeVisible()
  ...
  await expect(syncopationModule.getByRole('button', { name: '全程播放' })).toHaveClass(/active/)
  await syncopationModule.getByRole('button', { name: '仅预备拍' }).click()
  await expect(syncopationModule.getByRole('button', { name: '仅预备拍' })).toHaveClass(/active/)
  await syncopationModule.getByRole('button', { name: '全程播放' }).click()
  ...
  await syncopationModule.getByRole('button', { name: '正确节奏' }).click({ force: true })
  ...
  await syncopationModule.getByRole('button', { name: '三连音' }).click({ force: true })
  await expect(syncopationModule.getByText(/训练：.*三连音/)).toBeVisible()
  expect(await syncopationModule.locator('.jianpu-tuplet-arc').count()).toBeGreaterThan(0)
  ...
  await expect(page.getByRole('status')).toBeVisible({ timeout: 9000 })
...
test('layout fits mobile width', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '切分节奏跟拍' }).click()
  const syncopationModule = page.getByTestId('module-syncopation')
  await syncopationModule.getByRole('button', { name: '四十六节奏' }).click()
  const overflow = await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)
  expect(overflow).toBeLessThanOrEqual(2)
  const scoreBox = await syncopationModule.locator('.jianpu-score-svg').boundingBox()
  const moduleBox = await syncopationModule.boundingBox()
  expect(scoreBox?.width ?? 0).toBeLessThanOrEqual((moduleBox?.width ?? 0) + 1)
})
